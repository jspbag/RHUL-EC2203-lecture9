...
4. The regression $F$-statistic

# The multiple regression model

## The multiple regression model

$$Score_i = \beta_0 + \beta_1 STR_i  + \beta_2 PctEL_i + u_i$$

- $Score$ and $STR$ are test scores and class size, respectively

- $PctEL$ is pct of pupils that are English learners (control)

## The large sample distribution of the OLS estimator 

<div class="box">
If the **Least Squares assumptions** hold, then in large samples the OLS estimators $\hat{\beta}_0$, $\hat{\beta}_1$, and $\hat{\beta}_2$ are **jointly normally distributed**, and each $\hat{\beta}_j$ is distributed 

$$\mathcal{N}(\beta_j,\sigma_{\hat{\beta}_j}^2), \quad j = 0,1,2$$
</div>

- Use a **heteroskedasticity robust** estimator of $\sigma_{\hat{\beta}_j}^2$, call it $\hat{\sigma}_{\hat{\beta}_j}^2$
...
- Simple hypotheses are hypotheses that involve a **single restriction** on a **single coefficient**; for example,
...
- Joint hypotheses are hypotheses that involve **multiple restrictions** or **multiple coefficients**; for example,

  $$H_0: \, \beta_1 = \beta_3 = 0; \quad H_1:  \beta_1 \neq 0 \text{ or } \beta_3 \neq 0$$

- This joint hypothesis involves $q = 2$ restrictions
  
- Joint hypotheses cannot be tested using $t$-tests, use instead the $F$-test based on the $F$-statistic:

  $$F \sim F_{q,n-k-1} \approx F_{q,\infty},$$
  
  when $n$ is large (see pre-recorded lecture snippets)
...
- Compute the $F$-statistic and apply the rule:
  ...
  <div class="box">
  Reject $H_0$ if $F > F_{q,\infty}^{1-\alpha}$, where the critical value $F_{q,\infty}^{1-\alpha}$ is $(1-\alpha)\times 100$ percentile in the $F_{q,\infty}$-distribution
  </div>
...
```{r echo=TRUE, error=FALSE, warning=FALSE, out.width = "70%"}
lm2 <- lm(Score ~ STR + english + expenditure, data = CASchools)
linearHypothesis(lm2, c("STR=0", "expenditure=0"), white.adjust = "hc1")
```
...
```{r echo=TRUE, error=FALSE, warning=FALSE, out.width = "70%"}
lm2 <- lm(Score ~ STR + english + expenditure, data = CASchools)
linearHypothesis(lm2, c("STR=0"), white.adjust = "hc1")
```
...
- This joint hypothesis involves $q = 1$ restriction, but involves **two coefficients**
  
- This type of joint hypothesis can also be tested using an $F$-test (again, see pre-recorded lecture snippets for details)

## The $F$-test of a single restriction involving two coefficients

```{r echo=TRUE, error=FALSE, warning=FALSE, out.width = "70%"}
lm2 <- lm(Score ~ STR + english + expenditure, data = CASchools)
linearHypothesis(lm2, c("STR=expenditure"), white.adjust = "hc1")
```
...
```{r echo=TRUE, error=FALSE, warning=FALSE, out.width = "70%"}
lm2 <- lm(Score ~ STR + english + expenditure, data = CASchools)
linearHypothesis(lm2, c("STR=0", "english", "expenditure=0"))
```
...
```{r echo=TRUE, error=FALSE, warning=FALSE, out.width = "70%"}
lm2 <- lm(Score ~ STR + english + expenditure, data = CASchools)
linearHypothesis(lm2, c("STR=0", "english", "expenditure=0"), white.adjust = "hc1")
```
...
- Test of simple hypotheses in the multiple regression model can be done using the $t$-test

- The multiple regression model allows joint hypotheses: hypotheses involving multiple restrictions or multiple coefficients
